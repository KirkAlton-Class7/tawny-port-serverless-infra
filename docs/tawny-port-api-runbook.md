...
| --- | --- | --- | --- |
| Cellar | `/prod/cellar/*` | Auth0 JWT authorizer | Internal developer and machine-to-machine API access |
...
| Flow | Implementation |
| --- | --- |
| Cellar | Auth0 M2M token -> `Authorization: Bearer <TOKEN>` -> API Gateway JWT authorizer -> Cellar Lambda |
...
| Issuer URL | `https://<AUTH0_TENANT>.<AUTH0_REGION>.auth0.com/` |
...
| `python-sipper` | Reads `sessionId` from HTTP API cookies or REST-style Cookie header fallback and validates against DynamoDB |
| `node-sipper` | Mirrors Python sipper logic with AWS SDK v3 DynamoDB client |

> [!NOTE]
> HTTP API events expose cookies through `event.cookies`. The sipper and logout code also keep a REST API header fallback so the functions remain understandable across both API Gateway event shapes.
...
| --- | --- |
| API Gateway | HTTP APIs, route design, stages, JWT authorizers, Lambda integrations |
...
| Topic | References |
| --- | --- |
| API Gateway HTTP APIs | [HTTP APIs](https://docs.aws.amazon.com/apigateway/latest/developerguide/http-api.html), [Lambda proxy integrations](https://docs.aws.amazon.com/apigateway/latest/developerguide/http-api-develop-integrations-lambda.html) |
| JWT authorizers | [API Gateway JWT authorizers](https://docs.aws.amazon.com/apigateway/latest/developerguide/http-api-jwt-authorizer.html) |
| Lambda authorizers | [API Gateway HTTP API Lambda authorizers](https://docs.aws.amazon.com/apigateway/latest/developerguide/http-api-lambda-authorizer.html) |
...
- [ ] Auth0 JWT authorizer is attached only to Cellar routes.
