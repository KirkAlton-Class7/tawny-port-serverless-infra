...
**Purpose: Build Tawny Port, a serverless AWS infrastructure demo with API Gateway (REST), Lambda, Cognito Hosted UI, Auth0 M2M auth, and DynamoDB-backed sessions.**
...
| API Gateway API name | `tawny-port-rest` | No, unless renaming the implementation |
...
| --- | --- |
| REST API ID | `<API_ID>` |
| Root resource ID | Generated by API Gateway |
| Invoke URL after deployment | `https://<API_ID>.execute-api.<AWS_REGION>.amazonaws.com/prod` |
