...
def get_cookie(event, cookie_name):
    """Read a cookie from REST API headers, with HTTP API compatibility fallback."""
    ...
    for cookie_str in event.get('cookies', []) or []:
...
def lambda_handler(event, context):
    ...
    SOMMELIER_URL = os.environ.get(
        'SOMMELIER_URL',
        BASE_URL.rsplit('/chalice', 1)[0] + '/table/sommelier'
    )
    ...
    expected_csrf = get_cookie(event, 'oauth_state')
    ...
    if not expected_csrf or csrf_from_cognito != expected_csrf:
        ...
        return {
            ...
            <body style="font-family: Arial; text-align: center; margin-top: 50px;">
                ...
                <p><a href="''' + SOMMELIER_URL + '''">Click here to return to the Sommelier and try again</a></p>
            </body>
            </html>
            '''
        }
    ...
    return {
        ...
        'multiValueHeaders': {
            'Set-Cookie': [
                'oauth_state=; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=0; Expires=Thu, 01 Jan 1970 00:00:00 GMT',
                f'sessionId={session_id}; Domain={COOKIE_DOMAIN}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=3600'
            ]
        },
        'body': ''
    }
