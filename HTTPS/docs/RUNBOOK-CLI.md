# Tawny Port - HTTP API CLI Runbook

## Purpose

Build the Tawny Port HTTP API deployment with AWS CLI commands, then validate the browser user experience through Cognito Managed Login.
...
- API Gateway HTTP API routes, integrations, and authorizer placement
...
| Dependency | Requirement |
| --- | --- |
| AWS account and region | Create the Tawny Port HTTP API deployment in the intended account and region. |
...
| Dependency | Requirement |
| --- | --- |
| API Gateway HTTP API | Create routes, integrations, stages, and authorizer placement for this deployment. |
...
| [`../README.md`](../README.md) | HTTP API deployment overview and document map. |
...
```bash
cp HTTPS/env.example HTTPS/.env
source HTTPS/.env
```
---
...
Update these initial values in `HTTPS/.env`, then reload the file:
...
Package each repository source file with a Lambda-safe handler filename.

```bash
rm -rf HTTPS/build
mkdir -p HTTPS/build/zips

for name in python-cask sommelier auth-callback cognito-logout python-sipper; do
  mkdir -p "HTTPS/build/$name"
  cp "HTTPS/project-assets/lambda-code/$name.py" "HTTPS/build/$name/lambda_function.py"
  (cd "HTTPS/build/$name" && zip -qr "../zips/$name.zip" .)
done

for name in node-barrel node-sipper; do
  mkdir -p "HTTPS/build/$name"
  cp "HTTPS/project-assets/lambda-code/$name.cjs" "HTTPS/build/$name/index.cjs"
  (cd "HTTPS/build/$name" && zip -qr "../zips/$name.zip" .)
done

ls -lh HTTPS/build/zips
```

Expected:

```text
Seven Lambda ZIP files exist under HTTPS/build/zips.
```
...
```bash
aws lambda create-function --function-name "$PYTHON_CASK_FUNCTION" --runtime python3.12 --handler lambda_function.lambda_handler --role "$PYTHON_CASK_ROLE_ARN" --zip-file fileb://HTTPS/build/zips/python-cask.zip --region "$AWS_REGION"
aws lambda create-function --function-name "$SOMMELIER_FUNCTION" --runtime python3.12 --handler lambda_function.lambda_handler --role "$SOMMELIER_ROLE_ARN" --zip-file fileb://HTTPS/build/zips/sommelier.zip --region "$AWS_REGION"
aws lambda create-function --function-name "$AUTH_CALLBACK_FUNCTION" --runtime python3.12 --handler lambda_function.lambda_handler --role "$AUTH_CALLBACK_ROLE_ARN" --zip-file fileb://HTTPS/build/zips/auth-callback.zip --region "$AWS_REGION"
aws lambda create-function --function-name "$COGNITO_LOGOUT_FUNCTION" --runtime python3.12 --handler lambda_function.lambda_handler --role "$COGNITO_LOGOUT_ROLE_ARN" --zip-file fileb://HTTPS/build/zips/cognito-logout.zip --region "$AWS_REGION"
aws lambda create-function --function-name "$PYTHON_SIPPER_FUNCTION" --runtime python3.12 --handler lambda_function.lambda_handler --role "$PYTHON_SIPPER_ROLE_ARN" --zip-file fileb://HTTPS/build/zips/python-sipper.zip --region "$AWS_REGION"

aws lambda create-function --function-name "$NODE_BARREL_FUNCTION" --runtime nodejs20.x --handler index.handler --role "$NODE_BARREL_ROLE_ARN" --zip-file fileb://HTTPS/build/zips/node-barrel.zip --region "$AWS_REGION"
aws lambda create-function --function-name "$NODE_SIPPER_FUNCTION" --runtime nodejs20.x --handler index.handler --role "$NODE_SIPPER_ROLE_ARN" --zip-file fileb://HTTPS/build/zips/node-sipper.zip --region "$AWS_REGION"
```
...
export API_ENDPOINT="https://${API_ID}.execute-api.${AWS_REGION}.amazonaws.com/${API_STAGE}"
export API_HOST="${API_ID}.execute-api.${AWS_REGION}.amazonaws.com"
...
printf '\nexport API_ID=\"%s\"\nexport API_ENDPOINT=\"%s\"\nexport API_HOST=\"%s\"\nexport CALLBACK_REDIRECT_URI=\"%s\"\nexport POST_LOGOUT_REDIRECT_URI=\"%s\"\nexport BASE_CHALICE_URL=\"%s\"\n' \
  "$API_ID" "$API_ENDPOINT" "$API_HOST" "$CALLBACK_REDIRECT_URI" "$POST_LOGOUT_REDIRECT_URI" "$BASE_CHALICE_URL" >> HTTPS/.env
...
printf '\nexport COGNITO_USER_POOL_ID=\"%s\"\nexport COGNITO_APP_CLIENT_ID=\"%s\"\nexport COGNITO_CLIENT_SECRET=\"%s\"\nexport COGNITO_DOMAIN=\"%s\"\n' \
  "$COGNITO_USER_POOL_ID" "$COGNITO_APP_CLIENT_ID" "$COGNITO_CLIENT_SECRET" "$COGNITO_DOMAIN" >> HTTPS/.env
...
aws lambda update-function-configuration \
  ...
  --environment "Variables={BASE_URL=$BASE_CHALICE_URL,CLIENT_ID=$COGNITO_APP_CLIENT_ID,CLIENT_SECRET=$COGNITO_CLIENT_SECRET,COGNITO_DOMAIN=$COGNITO_DOMAIN,REDIRECT_URI=$CALLBACK_REDIRECT_URI,SESSION_TABLE=$SESSION_TABLE,COOKIE_DOMAIN=$API_HOST}" \
  --region "$AWS_REGION"
...
## 9. Configure HTTP API Integrations, Routes, And Authorizer

Create integrations:

```bash
export PYTHON_CASK_INTEGRATION_ID=$(aws apigatewayv2 create-integration --api-id "$API_ID" --integration-type AWS_PROXY --integration-uri "$PYTHON_CASK_ARN" --payload-format-version "2.0" --query 'IntegrationId' --output text --region "$AWS_REGION")
export NODE_BARREL_INTEGRATION_ID=$(aws apigatewayv2 create-integration --api-id "$API_ID" --integration-type AWS_PROXY --integration-uri "$NODE_BARREL_ARN" --payload-format-version "2.0" --query 'IntegrationId' --output text --region "$AWS_REGION")
export SOMMELIER_INTEGRATION_ID=$(aws apigatewayv2 create-integration --api-id "$API_ID" --integration-type AWS_PROXY --integration-uri "$SOMMELIER_ARN" --payload-format-version "2.0" --query 'IntegrationId' --output text --region "$AWS_REGION")
export AUTH_CALLBACK_INTEGRATION_ID=$(aws apigatewayv2 create-integration --api-id "$API_ID" --integration-type AWS_PROXY --integration-uri "$AUTH_CALLBACK_ARN" --payload-format-version "2.0" --query 'IntegrationId' --output text --region "$AWS_REGION")
export COGNITO_LOGOUT_INTEGRATION_ID=$(aws apigatewayv2 create-integration --api-id "$API_ID" --integration-type AWS_PROXY --integration-uri "$COGNITO_LOGOUT_ARN" --payload-format-version "2.0" --query 'IntegrationId' --output text --region "$AWS_REGION")
export PYTHON_SIPPER_INTEGRATION_ID=$(aws apigatewayv2 create-integration --api-id "$API_ID" --integration-type AWS_PROXY --integration-uri "$PYTHON_SIPPER_ARN" --payload-format-version "2.0" --query 'IntegrationId' --output text --region "$AWS_REGION")
export NODE_SIPPER_INTEGRATION_ID=$(aws apigatewayv2 create-integration --api-id "$API_ID" --integration-type AWS_PROXY --integration-uri "$NODE_SIPPER_ARN" --payload-format-version "2.0" --query 'IntegrationId' --output text --region "$AWS_REGION")
```

Create the Auth0 JWT authorizer:

```bash
export AUTH0_AUTHORIZER_ID=$(aws apigatewayv2 create-authorizer \
  --api-id "$API_ID" \
  --name tawny-port-auth0-jwt \
  --authorizer-type JWT \
  --identity-source '$request.header.Authorization' \
  --jwt-configuration Audience="$AUTH0_AUDIENCE",Issuer="$AUTH0_ISSUER" \
  --query 'AuthorizerId' \
  --output text \
  --region "$AWS_REGION")
```
...
```bash
aws apigatewayv2 create-route --api-id "$API_ID" --route-key 'GET /cellar/python-cask' --target "integrations/$PYTHON_CASK_INTEGRATION_ID" --authorization-type JWT --authorizer-id "$AUTH0_AUTHORIZER_ID" --region "$AWS_REGION"
aws apigatewayv2 create-route --api-id "$API_ID" --route-key 'GET /cellar/node-barrel' --target "integrations/$NODE_BARREL_INTEGRATION_ID" --authorization-type JWT --authorizer-id "$AUTH0_AUTHORIZER_ID" --region "$AWS_REGION"
aws apigatewayv2 create-route --api-id "$API_ID" --route-key 'GET /table/sommelier' --target "integrations/$SOMMELIER_INTEGRATION_ID" --region "$AWS_REGION"
aws apigatewayv2 create-route --api-id "$API_ID" --route-key 'GET /table/auth/callback' --target "integrations/$AUTH_CALLBACK_INTEGRATION_ID" --region "$AWS_REGION"
aws apigatewayv2 create-route --api-id "$API_ID" --route-key 'GET /table/auth/logout' --target "integrations/$COGNITO_LOGOUT_INTEGRATION_ID" --region "$AWS_REGION"
aws apigatewayv2 create-route --api-id "$API_ID" --route-key 'GET /chalice/python-sipper' --target "integrations/$PYTHON_SIPPER_INTEGRATION_ID" --region "$AWS_REGION"
aws apigatewayv2 create-route --api-id "$API_ID" --route-key 'GET /chalice/node-sipper' --target "integrations/$NODE_SIPPER_INTEGRATION_ID" --region "$AWS_REGION"
```

Allow API Gateway to invoke each Lambda:

```bash
for fn in "$PYTHON_CASK_FUNCTION" "$NODE_BARREL_FUNCTION" "$SOMMELIER_FUNCTION" "$AUTH_CALLBACK_FUNCTION" "$COGNITO_LOGOUT_FUNCTION" "$PYTHON_SIPPER_FUNCTION" "$NODE_SIPPER_FUNCTION"; do
  aws lambda add-permission \
    --function-name "$fn" \
    --statement-id "AllowExecutionFrom-${API_ID}-${fn}" \
...
| Cellar route returns 401 or 403 with token | Auth0 issuer or audience mismatch | Inspect JWT `iss` and `aud`, then update the HTTP API authorizer |
| Lambda returns wrapped HTML JSON | Route integration is not Lambda proxy style | Recreate or update the integration as `AWS_PROXY` |
